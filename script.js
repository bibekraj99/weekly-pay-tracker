...
function loadWeeklyData() {
    const savedData = JSON.parse(localStorage.getItem("weeklyData"));
    if (savedData) {
        weeklyData = savedData; // Load the saved data into the global `weeklyData`
    } else {
        weeklyData = {}; // Initialize empty if no saved data
    }
}

function storeDayData(date) {
    const hoursInput = document.getElementById(date).value;
    weeklyData[date] = parseFloat(hoursInput) || 0; // Store the input value
    localStorage.setItem("weeklyData", JSON.stringify(weeklyData)); // Save to localStorage
}
...
function initializeWeek() {
    ...
    daysOfWeek.forEach((day, index) => {
        ...
        currentDate.setDate(startDate.getDate() + index);
        const dateString = currentDate.toISOString().split("T")[0];
        const savedHours = weeklyData[dateString] || ''; // Load saved hours, default to empty
        daysContainer.innerHTML += `
            <div class="form-group">
                <label for="${dateString}">${day} (${dateString}) Hours:</label>
                <input type="number" id="${dateString}" placeholder="Enter hours for ${day}" min="0" value="${savedHours}" onchange="storeDayData('${dateString}')">
...
document.addEventListener("DOMContentLoaded", () => {
    loadWeeklyData(); // Load saved data from localStorage
    updateCalendar(); // Update the week range display
    initializeWeek(); // Populate the input fields with saved data
    updateHistory(); // Load and display the history
});
